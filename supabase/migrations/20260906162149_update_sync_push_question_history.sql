-- =============================================================
-- Migration: 20260906162149_update_sync_push_question_history.sql
--
-- Update question_history table to include mode, direction, and fsrs_log
-- Update sync_push RPC to correctly insert these new fields.
-- =============================================================

ALTER TABLE public.question_history ADD COLUMN IF NOT EXISTS mode TEXT;
ALTER TABLE public.question_history ADD COLUMN IF NOT EXISTS direction TEXT;
ALTER TABLE public.question_history ADD COLUMN IF NOT EXISTS fsrs_log TEXT;

CREATE INDEX IF NOT EXISTS question_history_user_answered_idx ON public.question_history(user_id, answered_at DESC);
CREATE INDEX IF NOT EXISTS question_history_user_concept_idx ON public.question_history(user_id, concept_id);
    ...
    begin
      if entity = 'sessions_log' then
        ...
        insert into public.question_history
          (op_id, user_id, concept_id, session_id, grade, response_ms, correct, answered_at, client_id, updated_at, mode, direction, fsrs_log)
        values (op_id_v, uid,
