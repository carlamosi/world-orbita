...
import { G as Globe } from '../_libs/react-globe.gl.mjs';
import { t as MeshPhongMaterial, h as Color, ai as TOUCH } from '../_libs/three.mjs';
import { f as feature } from '../_libs/topojson-client.mjs';
import { P as Plus, k as Minus, l as RotateCcw } from '../_libs/lucide-react.mjs';
import '../_libs/react-kapsule.mjs';
import '../_libs/jerrypick.mjs';
import '../_libs/globe.gl.mjs';
import '../_libs/three-globe.mjs';
import '../_libs/kapsule.mjs';
import '../_libs/lodash-es.mjs';
import '../_libs/tweenjs__tween.js.mjs';
import '../_libs/three-slippy-map-globe.mjs';
import '../_libs/d3-octree.mjs';
...
import '../_libs/three-geojson-geometry.mjs';
import '../_libs/earcut.mjs';
import '../_libs/accessor-fn.mjs';
import '../_libs/tinycolor2.mjs';
import '../_libs/data-bind-mapper.mjs';
import '../_libs/frame-ticker.mjs';
import '../_libs/three-conic-polygon-geometry.mjs';
import '../_libs/delaunator.mjs';
...
function loadCountryFeatures(resolution = "110m") {
  const cached = cache.get(resolution);
  if (cached) return cached;
  if (resolution === "110m") {
    const features = topoToFeatures(topo110);
...
function computeCentroid(geom) {
  ...
  for (const ring of iterRings(geom)) {
    ...
    for (let i = 0; i < len; i++) {
      const v = ring[i];
      const lng2 = v[0] * (Math.PI / 180);
      const lat2 = v[1] * (Math.PI / 180);
      const cl = Math.cos(lat2);
      x += cl * Math.cos(lng2);
      y += cl * Math.sin(lng2);
      z += Math.sin(lat2);
      n++;
...
function computeAngularSpan(geom, centroid) {
  const [clng, clat] = centroid;
  let maxDeg = 0;
  for (const ring of iterRings(geom)) {
    for (const v of ring) {
      const d = angularDistanceDeg$1(clat, clng, v[1], v[0]);
      if (d > maxDeg) maxDeg = d;
    }
  }
  return maxDeg;
}
function angularDistanceDeg$1(lat1, lng1, lat2, lng2) {
...
function computeArea(geom) {
  ...
  const wrap = (lst) => {
    ...
    for (let i = 0; i < lst.length - 1; i++) {
      const a = lst[i];
      const b = lst[i + 1];
      s += (b[0] - a[0]) * Math.sin((a[1] + b[1]) / 2 * (Math.PI / 180));
    }
    return Math.abs(s);
  };
  if (geom.type === "Polygon") {
...
const COLOR_HIGHLIGHT = "0, 255, 178";
const COLOR_REVEAL = "255, 107, 107";
const COLOR_DUE = "255, 184, 77";
const COLOR_HOVER = "0, 212, 255";
const COLOR_BASE = "108, 99, 255";
...
function strokeOpacityFor(alt) {
  const t = Math.max(0, Math.min(1, (2.4 - alt) / 2));
...
function Globe3D({
  ...
  const [dim, setDim] = reactExports.useState({ w: 600, h: 600 });
  const [hoverIso3, setHoverIso3] = reactExports.useState(null);
  const [altBand, setAltBand] = reactExports.useState(4);
  const [features, setFeatures] = reactExports.useState(
    () => loadCountryFeatures("110m")
  );
  const effectiveQuality = reactExports.useMemo(() => {
  ...
  const globeMaterial = reactExports.useMemo(() => {
  ...
  reactExports.useEffect(() => () => globeMaterial.dispose(), [globeMaterial]);
  const continentByIso3 = reactExports.useMemo(() => {
    const m = /* @__PURE__ */ new Map();
    for (const c of countries) m.set(c.iso3, c.continent);
    return m;
  }, [countries]);
  const countryByIso3 = reactExports.useMemo(() => {
    const m = /* @__PURE__ */ new Map();
    for (const c of countries) m.set(c.iso3, c);
    return m;
  }, [countries]);
  const featureByIso3 = reactExports.useMemo(() => {
    const m = /* @__PURE__ */ new Map();
    if (features) for (const f of features) m.set(f.properties.iso3, f);
    return m;
  }, [features]);
  const MICRO_AREA_KM2 = 1e3;
  const hitboxPoints = reactExports.useMemo(() => {
    const result = [];
    for (const c of countries) {
      ...
      const radius = altBand >= 4 ? 3 : altBand >= 3 ? 2.6 : altBand >= 2 ? 2.2 : altBand >= 1 ? 1.9 : 1.8;
      result.push({
        iso3: c.iso3,
        name: c.name,
        lat: c.coordinates[0],
        // Country dataset gives [lat, lng]
        lng: c.coordinates[1],
        radius,
        micro: true
      });
  ...
  const strokeOpacity = strokeOpacityFor(
    altBand === 0 ? 0.3 : altBand === 1 ? 0.55 : altBand === 2 ? 0.9 : altBand === 3 ? 1.4 : altBand === 4 ? 2 : 2.8
  );
  const dueSet = reactExports.useMemo(
  ...
  const pointColorFn = reactExports.useCallback(
    (d) => {
      const p = d;
  ...
  const pointLabelFn = reactExports.useCallback(
    (d) => {
      const p = d;
      if (!p.micro) return "";
  ...
  const pointRadiusFn = reactExports.useCallback((d) => {
    const p = d;
    if (!p.micro) return 0;
    const base = altBand >= 3 ? 0.42 : altBand >= 1 ? 0.36 : 0.3;
    return base;
  }, [altBand]);
  const pointAltitudeFn = reactExports.useCallback((d) => {
    const p = d;
  ...
  const polygonCapColor = reactExports.useCallback(
    (d) => {
      const f = d;
      ...
      if (iso3 === effHoverIso3) {
        const cont2 = continentByIso3.get(iso3);
        if (activeContinent && activeContinent !== "All" && cont2 !== activeContinent) {
  ...
  const polygonSideColor = reactExports.useCallback(
    () => `rgba(${COLOR_BASE}, 0.06)`,
    []
  );
  const polygonStrokeColor = reactExports.useCallback(
    (d) => {
      const f = d;
      const iso3 = f.properties.iso3;
  ...
  const polygonLabel = reactExports.useCallback(
    (d) => {
  ...
  const activeCameraTarget = reactExports.useMemo(() => {
    if (revealIso3) {
      ...
      if (c) {
        ...
        const altitude = Math.min(Math.max(liveAlt, 1), 1.6);
    ...
    if (focusIso3) {
      ...
      if (f) {
        const [clng, clat] = f.properties.centroid;
        const span = f.properties.angularSpan ?? 5;
        const altitude = Math.max(0.22, Math.min(2, Math.max(span, 1.5) / 28));
  ...
  const lastCameraTargetKey = reactExports.useRef(null);
  reactExports.useEffect(() => {
  ...
  const lastQuestionKey = reactExports.useRef(null);
  reactExports.useEffect(() => {
  ...
  const handlePolygonClick = reactExports.useCallback(
    (d) => {
      const f = d;
      const iso3 = f.properties.iso3;
      if (activeContinent && activeContinent !== "All") {
  ...
  const handlePolygonHover = reactExports.useCallback(
    (d) => {
      if (!d) {
        setHoverIso3(null);
        return;
  ...
  const handleHitboxClick = reactExports.useCallback(
    (d) => {
      const p = d;
      if (activeContinent && activeContinent !== "All") {
  ...
  const handleHitboxHover = reactExports.useCallback(
    (d) => {
      if (!d) {
        setHoverIso3(null);
        return;
  ...
  const lastBandRef = reactExports.useRef(altBand);
  ...
  const rings = reactExports.useMemo(() => {
    if (effectiveQuality === "static") return [];
  ...
  const zoomBy = reactExports.useCallback(
    (factor) => {
      const g = ref.current;
      if (!g) return;
      const pov = g.pointOfView();
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          Globe,
          {
            ...
            atmosphereAltitude: effectiveQuality === "static" ? 0.16 : effectiveQuality === "medium" ? 0.2 : 0.22,
            rendererConfig: {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "absolute top-3 right-3 z-10 flex flex-col gap-1.5 pointer-events-none", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(ZoomButton, { onClick: () => zoomBy(0.7), label: "Zoom in", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Plus, { className: "size-4", strokeWidth: 2.2 }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(ZoomButton, { onClick: () => zoomBy(1.4), label: "Zoom out", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Minus, { className: "size-4", strokeWidth: 2.2 }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(ZoomButton, { onClick: resetView, label: "Reset view", children: /* @__PURE__ */ jsxRuntimeExports.jsx(RotateCcw, { className: "size-4", strokeWidth: 2.2 }) })
        ] })
      ]
    }
  );
